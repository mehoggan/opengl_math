...
namespace opengl_math
{
  ...
  vector_4d<T> operator*(const matrix_4X4<T, ML> &m, const vector_4d<T> &v)
  {
    if (ML == column) {
      /* [col][row] */
      T a = m[0][0]*v[0]+m[1][0]*v[1]+m[2][0]*v[2]+m[3][0]*v[3];
      T b = m[0][1]*v[0]+m[1][1]*v[1]+m[2][1]*v[2]+m[3][1]*v[3];
      T c = m[0][2]*v[0]+m[1][2]*v[1]+m[2][2]*v[2]+m[3][2]*v[3];
      T d = m[0][3]*v[0]+m[1][3]*v[1]+m[2][3]*v[2]+m[3][3]*v[3];
      return vector_4d<T>(a, b, c, d);
    } else {
      /* [row][col] */
      T a = m[0][0]*v[0]+m[0][1]*v[1]+m[0][2]*v[2]+m[0][3]*v[3];
      T b = m[1][0]*v[0]+m[1][1]*v[1]+m[1][2]*v[2]+m[1][3]*v[3];
      T c = m[2][1]*v[0]+m[2][1]*v[1]+m[2][2]*v[2]+m[2][3]*v[3];
      T d = m[3][1]*v[0]+m[3][1]*v[1]+m[3][2]*v[2]+m[3][3]*v[3];
      return vector_4d<T>(a, b, c, d);
    }
  }

  /*! \brief This function transforms vector v b 4x4 matrix m
  */
  template <typename T, matrix_layout ML>
  vector_4d<T> operator*(const vector_4d<T> &v, const matrix_4X4<T, ML> &m)
  {
    if (ML == column) {
      /* [col][row] */
      T a = v[0]*m[0][0]+v[1]*m[0][1]+v[2]*m[0][2]+v[3]*m[0][3];
      T b = v[0]*m[1][0]+v[1]*m[1][1]+v[2]*m[1][2]+v[3]*m[1][3];
      T c = v[0]*m[2][0]+v[1]*m[2][1]+v[2]*m[2][2]+v[3]*m[2][3];
      T d = v[0]*m[3][0]+v[1]*m[3][1]+v[2]*m[3][2]+v[3]*m[3][3];
      return vector_4d<T>(a, b, c, d);
    } else if (ML == row) {
      /* [row][col] */
      T a = v[0]*m[0][0]+v[1]*m[1][0]+v[2]*m[2][0]+v[3]*m[3][0];
      T b = v[0]*m[0][1]+v[1]*m[1][1]+v[2]*m[2][1]+v[3]*m[3][1];
      T c = v[0]*m[0][2]+v[1]*m[1][2]+v[2]*m[2][2]+v[3]*m[3][2];
      T d = v[0]*m[0][3]+v[1]*m[1][3]+v[2]*m[2][3]+v[3]*m[3][3];
      return vector_4d<T>(a, b, c, d);
    }
  }
